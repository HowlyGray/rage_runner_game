...
var healing_time: float = 2.0
var despawn_timer: float = 8.0
...
const COMPLIMENT_COLOR = Color(1.0, 0.9, 0.0)
...
func setup(type: String, spawn_speed: float, heal_time: float, target_position: Vector3 = Vector3.ZERO):
	...
	if spawn_speed > 0 and target_position != Vector3.ZERO:
...
func set_compliment_appearance():
	...
	if mesh:
		...
		mat.albedo_color = COMPLIMENT_COLOR
...
func _process(delta):
	...
	if despawn_timer <= 2.0 and despawn_timer > 0:
		if mesh:
			mesh.visible = fmod(despawn_timer, 0.4) > 0.2
	if despawn_timer <= 0:
		queue_free()
